...
export type WelcomeShellProps = {
  ...
  tiers: number[];
  infoText?: ReactNode;
  /** 'absolute': goal = projectedTotal. 'additive': goal = currentTotal + projectedTotal. */
  mode: "absolute" | "additive";
  showFireworks?: boolean;
  fireworksDelayMs?: number;
...
const WelcomeShell = ({
  ...
  const [showRest, setShowRest] = useState(!showFireworks);

  useEffect(() => {
    if (!showFireworks) return;
    const t = window.setTimeout(() => setShowRest(true), fireworksDelayMs);
    return () => window.clearTimeout(t);
  }, [showFireworks, fireworksDelayMs]);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (mode === "additive") {
        ...
        goal = currentTotal + projectedTotal;
      }
      goal = Math.min(999999, goal);
  ...
  const tierTotal = (tier: number) => tier * daysRemaining;
  const customTotal = selected === "custom" ? projectedTotal : 0;

  return (
    <div
      className={cn(
        "min-h-screen bg-background flex flex-col items-center px-6 safe-top pt-12 pb-12",
        showFireworks && "relative overflow-hidden",
      )}
    >
      {showFireworks && (
      ...
      >
        ...
        <div
          className={cn(
            "text-center mb-8 flex flex-col items-center",
            showFireworks && "animate-fade-in",
          )}
        >
          <img src={smallCircleIcon} alt="" className="w-20 h-20 mb-4" />
          <h1 className="font-black text-white text-2xl">{headline}</h1>
          {showRest && (
            <>
              <p
                className={cn(
              ...
              >
                <span className="font-bold">{daysRemaining}d</span> remaining in{" "}
        ...
        {showRest && (
          <div className={cn(showFireworks && "animate-fade-in -mt-[14px]")}>
            {/* Tier options */}
                  ...
                  >
                    <div className="flex items-baseline justify-between">
                      <span
                        className={cn(
                          "text-lg",
                          isActive ? "text-[#0ABAB5]" : "text-white",
                        )}
                      >
                ...
                <div className="flex items-baseline justify-between gap-3">
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <span
                      className={cn(
                        "text-sm",
                        selected === "custom" ? "text-[#0ABAB5]" : "text-white",
                      )}
                    >
                      PU / day
                    </span>
            ...
            <p className="text-center text-muted-foreground/50 text-xs mt-8 flex items-center justify-center gap-1">
              ...
              <img src={muscleIcon} alt="" className="w-4 h-4 inline" />
            </p>
          </div>
        )}
      </div>
