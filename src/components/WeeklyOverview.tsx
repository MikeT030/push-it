...
const WeeklyOverview = () => {
  ...
  return <Collapsible open={isOpen} onOpenChange={setIsOpen} className="col-span-2">
      <div className="bg-card/40 rounded-2xl p-6 animate-slide-up pt-[10px] pb-[10px] mb-[10px] px-[10px]" style={{
      animationDelay: "0.25s"
    }}>
        <CollapsibleTrigger asChild>
          <button className="flex items-center justify-between w-full text-left hover:opacity-80 transition-opacity mb-[6px] mt-[10px] px-[10px] pt-[10px] pb-[10px]">
            <h2 className="text-lg font-bold text-foreground">Weekly</h2>
            {isOpen ? <ChevronDown className="w-5 h-5 text-muted-foreground" /> : <ChevronRight className="w-5 h-5 text-muted-foreground" />}
          </button>
        </CollapsibleTrigger>

        <div className="h-px mb-6 bg-[#3b404f]" />
      ...
      {/* Weekly Summary */}
      <div className="flex items-center justify-between mb-[6px] px-1" onClick={(e) => e.stopPropagation()}>
        <div>
